...
instance Semigroup (ReactNode sig) where
  (NodeSequence xs) <> (NodeSequence ys) = NodeSequence (xs <> ys)
  (NodeSequence xs) <> y = NodeSequence (xs <> [y])
  x <> (NodeSequence ys) = NodeSequence (x : ys)
  x <> y = NodeSequence [x, y]
